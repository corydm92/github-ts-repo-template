...
- `docs/blueprint/Layer 05 - Build & Delivery/5.2 — Trunk-Based + Feature Flag CI CD 🚦/Supporting Atomic Notes/Release Automation Behavior.md`
...
- If an architectural change has no ADR, propose drafting one before implementation.
- If a change conflicts with an existing ADR, propose a new ADR that supersedes it.
...
- Add new, high-value documentation in the appropriate Blueprint layer/section (do not leave it in Section Example).

Examples:

- `.github/ISSUE_TEMPLATE/*.md`
- Trunk-Based + Feature Flag CI/CD Deployment Flow (Docker/Vercel/npm): `docs/blueprint/Layer 05 - Build & Delivery/5.2 — Trunk-Based + Feature Flag CI CD 🚦/Base Project Rules and Tooling/Trunk-Based + Feature Flag CI-CD Deployment Flow (Docker-Vercel-npm).md`
...
- Enforce best practices: if a request skips critical safeguards or standards, push back and propose fixes (e.g., cross-app import boundaries).
...
- Use real newlines (heredoc/printf) when creating PRs; never embed `\n` escapes in the body string.
...
- Release Automation Behavior: `docs/blueprint/Layer 05 - Build & Delivery/5.2 — Trunk-Based + Feature Flag CI CD 🚦/Supporting Atomic Notes/Release Automation Behavior.md`
