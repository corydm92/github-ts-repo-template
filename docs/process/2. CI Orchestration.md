...
Deep dive: `scripts/README.md` (script flow + change detection contract)
...
- `pnpm run ci:force` → full run (all apps + all packages)
- `pnpm -C apps/<app> run ci` → just one app
- `pnpm -C packages/<pkg> run ci` → just one package

Daily safety net:

- Workflow: `.github/workflows/ci-daily-enforce-check.yml`
- Schedule: daily 14:00 UTC (6:00 AM PT, accepts DST shift)
- Command: `pnpm run ci:force`
